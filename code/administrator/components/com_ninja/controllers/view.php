...
/**
 * @version		$Id: view.php 1050 2011-06-16 10:41:24Z richie $
 * @package		Ninja
...
class ComNinjaControllerView extends ComDefaultControllerDefault
{
	...
	/**
	 * Legacy function! Don't use this, use registerCallback instead
	 *
	 * @author Stian Didriksen <stian@ninjaforge.com>
	 * @param $name
	 * @param $callback
	 */
	public function registerFunctionBefore($names, $functions)
	{
		return $this->registerFunction('before', $names, $functions);
	}
	 ...
	 * @param $callback
	 */
	public function registerFunctionAfter($names, $functions)
	{
		return $this->registerFunction('after', $names, $functions);
	}
	
	/**
	 * Used by the legacy functions registerFunctionBefore and registerFunctionAfter!
	 *
	 * @author Stian Didriksen <stian@ninjaforge.com>
	 * @param $when
	 * @param $name
	 * @param $callback
	 */
	private function registerFunction($when, $names, $functions)
	{
		$events = array();
		...
		{
			$this->registerCallback($events, array($this, $function));
		}
	 ...
	 * Unlock callback
	 *
	 * Overriding Nooku Framework here to make unlock work on save (rowset)
	 *
	...
	public function unlockRow(KCommandContext $context)
	{
		...
		if($row instanceof KDatabaseRowsetInterface) $row = $row->top();
