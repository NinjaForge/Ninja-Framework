 ...
 * GD Image Driver.
 *
 * $Id: gd.php 1023 2011-04-13 10:12:52Z stian $
 *
 * @package    Image
 * @author     Kohana Team
 ...
 */
class ComNinjaHelperImageDriverGd extends ComNinjaHelperImageDriver {
	...
	public function __construct()
	{
		...
		if ( ! function_exists('gd_info'))
            throw new KException('image gd requires v2');
	...
	public function process($image, $actions, $dir, $file, $render = FALSE)
	{
		...
		if (empty($create) OR ! function_exists($create))
			throw new KException('Image type "'.$image['type'].'" not allowed as the required function "'.$create.'" is missing.');
    ...
    public function flip($direction)
	{
		...
		if ($direction === ComNinjaHelperImage::HORIZONTAL)
		{
			for ($x = 0; $x < $width; $x++)
			{
		...
		elseif ($direction === ComNinjaHelperImage::VERTICAL)
		{
			for ($y = 0; $y < $height; $y++)
			{
	...
	public function resize($properties)
	{
		...
		if ($properties['master'] === ComNinjaHelperImage::AUTO)
		{
			// Change an automatic master dim to the correct type
			$properties['master'] = (($width / $properties['width']) > ($height / $properties['height'])) ? ComNinjaHelperImage::WIDTH : ComNinjaHelperImage::HEIGHT;
		}

		if (empty($properties['height']) OR $properties['master'] === ComNinjaHelperImage::WIDTH)
		{
			// Recalculate the height based on the width
			$properties['height'] = round($height * $properties['width'] / $width);
		}

		if (empty($properties['width']) OR $properties['master'] === ComNinjaHelperImage::HEIGHT)
		{
	...
	public function sharpen($amount)
	{
		...
		if ( ! function_exists('imageconvolution'))
			throw new KException('image unsupported method');
