 ...
 *
 * $Id: imagemagick.php 503 2010-09-20 19:00:21Z stian $
 *
 * @package    Image
 * @author     Kohana Team
 ...
 */
class ComNinjaHelperImageDriverImagemagick extends ComNinjaHelperImageDriver {
	...
	public function __construct($config)
	{
		if (empty($config['directory']))
		{
			...
			if ( ! is_file($path = exec('which convert')))
				throw new KException('image imagemagick not_found');
	...
	public function process($image, $actions, $dir, $file, $render = FALSE)
	{
		...
		JFile::copy($image, $this->tmp_image);
		...
		if ($status = $this->execute($actions))
		{
			...
			else
			{
				...
				if ($render !== FALSE)
				{
					$contents = JFile::read($this->tmp_image);
	...
	public function flip($dir)
	{
		...
		$dir = ($dir === ComNinjaHelperImage::HORIZONTAL) ? '-flop' : '-flip';
	...
	public function resize($prop)
	{
		switch ($prop['master'])
		{
			case ComNinjaHelperImage::WIDTH:  // Wx
				$dim = escapeshellarg($prop['width'].'x');
			break;
			case ComNinjaHelperImage::HEIGHT: // xH
				$dim = escapeshellarg('x'.$prop['height']);
			break;
			case ComNinjaHelperImage::AUTO:   // WxH
				$dim = escapeshellarg($prop['width'].'x'.$prop['height']);
			break;
			case ComNinjaHelperImage::NONE:   // WxH!
				$dim = escapeshellarg($prop['width'].'x'.$prop['height'].'!');
			break;
		}
