 ...
 */
class NinjaHelperImageDriverGd extends NinjaHelperImageDriver {
    ...
    public function flip($direction)
	{
		...
		if ($direction === NinjaHelperImage::HORIZONTAL)
		{
			for ($x = 0; $x < $width; $x++)
			{
		...
		elseif ($direction === NinjaHelperImage::VERTICAL)
		{
			for ($y = 0; $y < $height; $y++)
			{
	...
	public function resize($properties)
	{
		...
		if ($properties['master'] === NinjaHelperImage::AUTO)
		{
			// Change an automatic master dim to the correct type
			$properties['master'] = (($width / $properties['width']) > ($height / $properties['height'])) ? NinjaHelperImage::WIDTH : NinjaHelperImage::HEIGHT;
		}

		if (empty($properties['height']) OR $properties['master'] === NinjaHelperImage::WIDTH)
		{
			// Recalculate the height based on the width
			$properties['height'] = round($height * $properties['width'] / $width);
		}

		if (empty($properties['width']) OR $properties['master'] === NinjaHelperImage::HEIGHT)
		{
