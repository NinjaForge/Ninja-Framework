...
/**
 * @version		$Id: select.php 1015 2011-04-11 19:20:56Z stian $
 * @package		Ninja
 ...
 * @subpackage	Helper
 */
class ComNinjaHelperSelect extends KTemplateHelperSelect
{
    ...
    */
    ...
    {
       $timeformats = KFactory::get('admin::com.ninja.model.timeformats')->getList();
 		$date = KFactory::get('lib.joomla.utilities.date'); 
        // LOCALE SETTINGS
	...
	/**
	* Generates a enabled/disabled radio list
	*
	...
	*/
	public function statelist($config = array())
	{
		...
		$img_y = KFactory::get('admin::com.ninja.helper.default')->img($config->img_y);
		$img_x = KFactory::get('admin::com.ninja.helper.default')->img($config->img_x);
		$img = array(
			$img_y ? '<img src="' . $img_y . '" border="0" alt="' . JText::_( $config->yes ) . '" /> ' : null,
			$img_x ? '<img src="' . KFactory::get('admin::com.ninja.helper.default')->img($config->img_x) . '" border="0" alt="' . JText::_( $config->no ) . '" /> ' : null
		);
		$arr = array(
	...
	*/
	...
	public function images($config = array())//$path, $name, $attribs = null, $selected = NULL, $idtag = false, $translate = false, $vertical = false, $script = true)
	{
		$config = new KConfig($config);
		...
		$images		= KFactory::tmp('admin::com.ninja.model.images')->folder($config->path);
		...
		if(strpos($path, '/media') === 0) {
			...
			$images		= KFactory::tmp('admin::com.ninja.model.images')->folder($path);
	...
	?>
		<div class="group image-select images<?php echo $config->vertical ?>">
			<?php foreach($options as $i => $option) : ?>
				...
				<?php $id = KFactory::get('lib.koowa.filter.alnum')->sanitize($option->value) ?>
	...
	?>
		<div class="group images">
			...
			<div id="rank_image_preview"><img src="<?php echo KTemplateAbstract::loadHelper('admin::com.ninja.helper.default.image', '/16/spinner.gif') ?>" /></div>
