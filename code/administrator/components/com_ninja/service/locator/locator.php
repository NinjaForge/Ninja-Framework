 ...
 * @subpackage 	Locator
 */
class NinjaServiceLocator extends KServiceLocatorAbstract
{
	/** 
	 * The type
	 * 
	...
	{ 
        ...
        $classname = 'Ninja'.ucfirst($identifier->package).$path.ucfirst($identifier->name);

      	//Manually load the class to set the basepath
		if (!KService::get('koowa:loader')->loadClass($classname, $identifier->basepath))
		{
			 ...
			 *                     -> Framework Default
			 */
			if(class_exists('Ninja'.ucfirst($identifier->package).ucfirst($classtype).$path.ucfirst($identifier->name))) {
				$classname = 'Ninja'.ucfirst($identifier->package).ucfirst($classtype).$path.ucfirst($identifier->name);
			} elseif(class_exists('Ninja'.ucfirst($identifier->package).ucfirst($classtype).$path.'Default')) {
				$classname = 'Ninja'.ucfirst($identifier->package).ucfirst($classtype).$path.'Default';
	...
	public function findPath(KServiceIdentifier $identifier)
	{
		...
		array_unshift($parts, strtolower($identifier->package));
