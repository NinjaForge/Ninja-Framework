...
NPaginator = new Class({
	...
	initialize: function(paginator, options){
		...
		this.xhr = new XHR({method: 'get', onSuccess: function(data){
			var page = document.getElement('table.adminlist tbody').empty().setHTML(data);
			$(this.options.identificator).fireEvent('onInject');
	...
	inject: function(){
		...
		var query = value[0].parseUri().query;
		var href  = value[0].replace(window.location.search, '');
		var uris= query.parseQueryString();
		query = [];
		if(value.length > 1)
		{
		...
		if(this.cache[Json.toString(this.options.state)]){
			document.getElement('table.adminlist tbody').empty().setHTML(this.cache[Json.toString(this.options.state)]);
			$(this.options.identificator).fireEvent('onInject');
		} else {
			this.xhr.send(href, 'layout=default_items&format=ajax' + query);
		}
		this.update();
	},
...
NPaginator.implement(new Events, new Options);