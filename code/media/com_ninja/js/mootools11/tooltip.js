...
NTooltip = new Class({
	...
	options: {
		onShow: function(tip){
			...
			if(tip.getStyle('opacity')==0) tip.addClass('scale-in').effects({onStart: function(){ tip.setStyle('display', 'block'); }.bind(this)}).start({'opacity': 1});
		},
		onHide: function(tip){
			var top = tip.getPosition().y;
			var offset = this.options.fixed ? 20 : 0;
			tip.addClass('scale-out').effects({onComplete: function(){ tip.removeClass('scale-out').removeClass('scale-in').setStyle('display', 'none'); }}).start({'opacity': 0});
		},
	...
	start: function(el){
		if (el.$tmp.myText){
			...
			if(html != el.$tmp.myText)
			{
				this.toolTip.getElement('.middle span').empty();
				this.text = this.toolTip.getElement('.middle span').setHTML(el.$tmp.myText);
				el.$tmp.myText = this.toolTip.getElement('.middle span').innerHTML;
...
NTooltip.implement(new Events, new Options);